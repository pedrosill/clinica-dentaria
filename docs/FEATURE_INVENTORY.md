...
| Production security | Session auth, CSRF/origin checks, security headers, fail-closed production config, local encrypted backup/restore tooling, central role/resource matrix, dentist-to-doctor scope, non-destructive archiving, and audit history | Clinic must still approve deployment, retention, and incident procedures |
