...
| Clinical records | Medical profile, FDI odontogram, tooth findings, treatment plans, paper-transcription state, linked draft creation when a visit is concluded, immutable final notes, and dentist validation/finalization | `client/src/components/patient-detail/ClinicalRecordSection.jsx`, `server/src/services/clinicalService.js`, `server/src/services/appointmentService.js` |
...
| Clinical workflow | Patient clinical workspace, appointment-linked draft notes, final-note locking, versioned addenda, paper transcription state, and dentist validation/finalization | Expose richer note editing/addenda and a complete migration register with explicit batch status |
...
| Production security | Session auth, CSRF/origin checks, security headers, central role/resource matrix, MFA/recovery, private files, encrypted backup status and second-copy support, dentist scope, required audit for sensitive actions, and an explicit local-only binding profile | Active SQLite volume encryption, local firewall/OS account controls, OS scheduler/alerting, external backup host, and clinic approval remain operational requirements |
