...
| Clinical records | Medical profile, FDI odontogram, tooth findings, clinical notes, treatment plans, paper-transcription state, and dentist validation endpoint | `client/src/components/patient-detail/ClinicalRecordSection.jsx`, `server/src/services/clinicalService.js` |
...
| Clinical workflow | Patient clinical workspace, final-note locking, versioned addenda, paper transcription state, and dentist validation endpoint | Link appointment conclusion to a draft/final clinical note and expose note editing where allowed |
