...
| Production security | Session auth, CSRF/origin checks, security headers, central role/resource matrix, MFA/recovery, private files, encrypted backup status and second-copy support, dentist scope, required audit for sensitive actions, explicit loopback profile, and a fictional-data Docker profile for shared local-network testing | HTTPS proxy deployment for two clinic computers, active SQLite/VM volume encryption, local firewall/OS account controls, OS scheduler/alerting, external backup host, and clinic approval remain operational requirements |
