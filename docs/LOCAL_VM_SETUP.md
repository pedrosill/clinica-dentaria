...
O script instala Docker, clona o branch `master`, cria o `.env` local, pergunta se devem ser criados dados fictícios, gera uma chave de cifragem para os backups, pede a pasta da segunda cópia, configura um backup diário às 23:00 de Portugal e inicia os containers. Se o `.env` já tiver estas definições, preserva-as.

Para um teste simples, pode aceitar-se a criação dos dados fictícios e a pasta predefinida. Na clínica, responda `N` à pergunta dos dados fictícios e indique uma pasta num disco ou suporte separado e aprovado; o script não consegue escolher esse suporte automaticamente.

O timer `dentalpro-backup.timer` executa o backup cifrado e a verificação todos os dias. Para consultar o estado:

```bash
systemctl list-timers dentalpro-backup.timer
