...
const initialForm = { email: '', displayName: '', role: 'receptionist', password: '' };
  ...
  return (
    <section className="clinic-panel rounded-3xl p-6 md:p-8">
      ...
      <form onSubmit={handleCreateUser} className="mt-6 grid gap-3 md:grid-cols-2">
        <label className="space-y-2 text-sm font-medium text-slate-700">{t('Display name')}<input required name="displayName" autoComplete="name" className={inputClass} value={form.displayName} onChange={updateForm} disabled={isSaving} /></label>
        <label className="space-y-2 text-sm font-medium text-slate-700">{t('Email')}<input required type="email" name="email" autoComplete="email" className={inputClass} value={form.email} onChange={updateForm} disabled={isSaving} /></label>
