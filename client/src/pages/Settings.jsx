    ...
    <div className="w-full space-y-6">
      ...
      {activeSettingsSection === 'clinic' ? <section className="clinic-panel rounded-2xl p-5 md:p-6">
        ...
        {editingSettingsSection === 'clinic' && canManage ? <form onSubmit={handleSaveClinicSettings} autoComplete="off" className="mt-6">
          <div className="grid gap-4 md:grid-cols-2"><label className="space-y-2 text-sm font-medium text-slate-700">{t('Clinic name')}<input className={inputClass} value={clinicForm.clinicName} onChange={(event) => setClinicForm({ ...clinicForm, clinicName: event.target.value })} /></label><label className="space-y-2 text-sm font-medium text-slate-700">{t('Timezone')}<input className={inputClass} value={clinicForm.timezone} onChange={(event) => setClinicForm({ ...clinicForm, timezone: event.target.value })} /></label><label className="space-y-2 text-sm font-medium text-slate-700">{t('Language')}<select name="language" data-testid="language-select" className={inputClass} value={clinicForm.language} onChange={(event) => { const nextLanguage = event.target.value; hasUnsavedLanguageChange.current = true; setClinicForm((current) => ({ ...current, language: nextLanguage })); setLanguage(nextLanguage); }}>{LANGUAGE_OPTIONS.map((option) => <option key={option.value} value={option.value}>{t(option.label)}</option>)}</select></label></div>
          <div className="mt-6 overflow-x-auto rounded-xl border border-slate-200"><table className="min-w-full divide-y divide-slate-200 text-sm"><thead className="bg-slate-50"><tr><th className="px-4 py-3 text-left font-semibold text-slate-700">{t('Day')}</th><th className="px-4 py-3 text-left font-semibold text-slate-700">{t('Open')}</th><th className="px-4 py-3 text-left font-semibold text-slate-700">{t('Hours')}</th><th className="px-4 py-3 text-left font-semibold text-slate-700">{t('Break')}</th></tr></thead><tbody className="divide-y divide-slate-200 bg-white">{scheduleDraft.map((day) => { const label = WEEKDAYS.find((item) => item.weekday === day.weekday)?.label; return <tr key={day.weekday}><td className="whitespace-nowrap px-4 py-3 font-medium text-slate-900">{t(label)}</td><td className="px-4 py-3"><input type="checkbox" checked={day.isOpen} onChange={(event) => updateDay(day.weekday, { isOpen: event.target.checked })} className="h-4 w-4 accent-teal-700" aria-label={`${t(label)} ${t('Open')}`} /></td><td className="px-4 py-3"><div className="flex min-w-64 gap-2"><input type="time" step="1800" className={inputClass} value={day.startTime} onChange={(event) => updateDay(day.weekday, { startTime: event.target.value })} disabled={!day.isOpen} /><span className="self-center text-slate-400">{t('to')}</span><input type="time" step="1800" className={inputClass} value={day.endTime} onChange={(event) => updateDay(day.weekday, { endTime: event.target.value })} disabled={!day.isOpen} /></div></td><td className="px-4 py-3"><div className="flex min-w-64 gap-2"><input type="time" step="1800" className={inputClass} value={day.breakStart} onChange={(event) => updateDay(day.weekday, { breakStart: event.target.value })} disabled={!day.isOpen} /><span className="self-center text-slate-400">{t('to')}</span><input type="time" step="1800" className={inputClass} value={day.breakEnd} onChange={(event) => updateDay(day.weekday, { breakEnd: event.target.value })} disabled={!day.isOpen} /></div></td></tr>; })}</tbody></table></div>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
         ...
         <section className="clinic-panel rounded-2xl p-5 md:p-6">
          ...
          {editingSettingsSection === 'types' && canManage ? <form onSubmit={handleAddType} autoComplete="off" className="mt-6 grid gap-3 sm:grid-cols-[1fr_110px_auto]"><input required placeholder={t('Appointment type')} className={inputClass} value={typeForm.name} onChange={(event) => setTypeForm({ ...typeForm, name: event.target.value })} disabled={isSaving} /><select className={inputClass} value={typeForm.duration} onChange={(event) => setTypeForm({ ...typeForm, duration: event.target.value })} disabled={isSaving}>{[30, 60, 90, 120].map((duration) => <option key={duration} value={duration}>{duration} {t('min')}</option>)}</select><button type="submit" disabled={isSaving} className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-50 disabled:opacity-60"><Plus className="h-4 w-4" />{t('Add')}</button></form> : null}
          <div className="mt-5 space-y-2">{activeTypes.map((type) => editingSettingsSection === 'types' ? <div key={type.id} className="grid gap-2 rounded-xl border border-slate-200 bg-slate-50 p-3 sm:grid-cols-[1fr_110px_auto_auto]"><input className={inputClass} value={typeDrafts[type.id]?.name || ''} onChange={(event) => setTypeDrafts({ ...typeDrafts, [type.id]: { ...typeDrafts[type.id], name: event.target.value } })} disabled={isSaving} /><select className={inputClass} value={typeDrafts[type.id]?.duration || String(type.duration)} onChange={(event) => setTypeDrafts({ ...typeDrafts, [type.id]: { ...typeDrafts[type.id], duration: event.target.value } })} disabled={isSaving}>{[30, 60, 90, 120].map((duration) => <option key={duration} value={duration}>{duration} {t('min')}</option>)}</select><button type="button" onClick={() => handleSaveType(type.id)} disabled={isSaving} className="inline-flex items-center justify-center rounded-xl bg-teal-700 px-3 py-2 text-white hover:bg-teal-800 disabled:opacity-50" aria-label={`${t('Save')} ${type.name}`}><Save className="h-4 w-4" /></button><button type="button" onClick={() => handleArchiveType(type.id)} disabled={isSaving} className="inline-flex items-center justify-center rounded-xl border border-red-200 bg-white px-3 py-2 text-red-700 hover:bg-red-50 disabled:opacity-50" aria-label={`${t('Archive')} ${type.name}`}><Trash2 className="h-4 w-4" /></button></div> : <div key={type.id} className="flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50 px-3 py-3 text-sm"><span className="font-medium text-slate-800">{type.name}</span><span className="text-slate-600">{type.duration} {t('min')}</span></div>)}</div>
        </section>
      </div>
