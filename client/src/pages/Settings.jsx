...
export default function Settings() {
  ...
  return (
    <div className="w-full space-y-6">
      ...
      {activeSettingsSection === 'clinic' ? <form onSubmit={handleSaveClinicSettings} className="clinic-panel rounded-3xl p-6 md:p-8">
        <div className="flex items-start gap-3">
          ...
          <div><h2 className="text-xl font-semibold text-slate-900">{t('Practice schedule')}</h2><p className="mt-1 text-sm leading-6 text-slate-600">{t('These hours control every available start time in the booking engine.')}</p></div>
        </div>
      ...
      <label className="space-y-2 text-sm font-medium text-slate-700">{t('Language')}<select name="language" data-testid="language-select" className={inputClass} value={clinicForm.language} onChange={(event) => { const nextLanguage = event.target.value; hasUnsavedLanguageChange.current = true; setClinicForm((current) => ({ ...current, language: nextLanguage })); setLanguage(nextLanguage); }} disabled={!canManage}>{LANGUAGE_OPTIONS.map((option) => <option key={option.value} value={option.value}>{t(option.label)}</option>)}</select></label>
        ...
        <div className="mt-5 flex flex-wrap items-center justify-between gap-3"><p className="text-xs text-slate-500">{t('Appointment grid is fixed at 30 minutes. The latest valid start is calculated from the closing time and duration.')}</p><button data-testid="save-clinic-settings" type="submit" disabled={!canManage || isSaving} className="inline-flex items-center gap-2 rounded-xl bg-teal-700 px-4 py-2.5 text-sm font-medium text-white shadow-sm hover:bg-teal-800 disabled:cursor-not-allowed disabled:opacity-60"><Save className="h-4 w-4" />{t('Save schedule')}</button></div>
      </form> : null}
